...
from pdfutility.app import MainWindow
...
class FileDialogTests(unittest.TestCase):
    def test_uses_and_remembers_last_open_directory(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("pdfutility.app.QSettings", return_value=settings), patch(
                "pdfutility.app.QFileDialog.getOpenFileName",
    ...
    def test_missing_saved_directory_falls_back_to_default(self):
        ...
        with patch("pdfutility.app.QSettings", return_value=settings), patch(
            "pdfutility.app.QFileDialog.getOpenFileName", return_value=("", ""),
        ) as dialog:
            result = MainWindow.select_pdf(parent, "Open")

        self.assertEqual(result, "")
        dialog.assert_called_once_with(parent, "Open", "", "PDF (*.pdf)")
